...
/**
 * Create polyline and transformed intersection data.
 *
 * The data contains source polyline, i.e. polyline including extension length if any.
 * The intersection data contains the transformed fence mesh sections.
 */
export type PolylineAndIntersectionData = {
    sourcePolyline: PolylineWithSectionLengths; // TODO: Move to storedData for layer later on
    intersectionData: PolylineIntersection_trans;
};

/**
 * Assemble polyline and transformed intersection data.
 *
 * The transformed intersection data is decoded from base64 to typed arrays.
 */
export function createPolylineAndTransformedIntersectionData(
    sourcePolyline: PolylineWithSectionLengths,
    intersectionApiData: PolylineIntersection_api
): PolylineAndIntersectionData {
    const intersectionData = transformPolylineIntersection(intersectionApiData);
