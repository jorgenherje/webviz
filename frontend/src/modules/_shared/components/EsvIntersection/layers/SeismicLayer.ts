...
export class SeismicLayer extends CanvasLayer<SeismicLayerData> {
    ...
    override onUpdate(event: OnUpdateEvent<SeismicLayerData>): void {
        ...
        const adjustedColorScale = event.data.colorScale.clone();
        if (this._seismicInfo) {
            adjustedColorScale.setRangeAndMidPoint(this._seismicInfo.domain.min, this._seismicInfo.domain.max, 0);
        }
        ...
        this.generateImage(seismicSliceImageOptions, true).then((image) => {
            this._image = image;
            this.render();
        });
    }
