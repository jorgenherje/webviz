...
export class EnsembleVectorListsHelper {
    private _ensembleIdents: string[];
    private _queries: UseQueryResult<VectorDescription_api[]>[];

    constructor(ensembleIdents: string[], vectorListQueryResults: UseQueryResult<VectorDescription_api[]>[]) {
     ...
     */
    isVectorInEnsemble(ensembleIdent: string, vector: string): boolean {
     ...
     */
    hasHistoricalVector(ensembleIdent: string, vector: string): boolean {
     ...
     */
    private findIndexOfEnsembleIdent(ensembleIdent: string): number {
        return this._ensembleIdents.findIndex((ident) => {
            return ident === ensembleIdent;
        });
    }
}
